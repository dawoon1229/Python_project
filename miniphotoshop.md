...
- 파일 메뉴에서 '파일 열기'를 선택하면 파일 다이얼로그가 열려 사용자가 이미지를 선택할 수 있다.
- '파일 저장'을 선택하면 현재 작업 중인 이미지를 사용자가 지정한 경로에 저장한다.
  
### 이미지 처리

- 반전 이미지: 이미지의 색상을 반전시킨다.
- 밝기 조절: 사용자가 입력한 값에 따라 이미지의 밝기를 조절한다.
- 회색조 이미지: RGB 값을 평균내어 회색조 이미지로 변환한다.
- 흑백 이미지: 임계값을 기준으로 흑백 이미지로 변환한다.

### 이미지 변형

- 이미지 확대/축소: 사용자가 입력한 배율에 따라 이미지를 확대 또는 축소한다.
- 이미지 반전: 이미지를 상하 또는 좌우로 반전시킨다.
- 이미지 회전: 사용자가 선택한 각도에 따라 이미지를 회전시킨다.

---

## 5. 실행 및 테스트

- 프로그램을 실행하고 '파일 열기'를 통해 이미지를 불러온 후, 각 기능을 테스트한다.
- 이미지 처리, 변형 기능을 테스트하여 정상적으로 작동하는지 확인한다.
- 다양한 이미지에 대해 테스트하여 예외 상황에 대비한다.

---

## 6. 결론
이 프로그램은 간단한 이미지 처리 및 변형 기능을 제공하여 사용자가 이미지를 다양하게 가공할 수 있도록 도와준다. 사용자 편의성을 높이기 위해 GUI를 사용하고, 각 기능은 사용자 입력에 따라 동적으로 처리된된다. 추가적인 기능이나 사용성 개선을 위해 피드백을 받고 지속적인 업데이트를 진행할 수 있다.
